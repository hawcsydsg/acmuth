...
// 接着，定义了一个立即执行函数，并在函数内部进行了一些操作。首先，通过document.getElementsByClassName('shell2')[0]获取到具有shell2类名的元素，并将其存储在shell2变量中。然后，通过shell2.getElementsByClassName方法获取到该shell2元素下的shell2_slider、item、prev和next元素，并将它们分别存储在slider、items、prevBtn和nextBtn变量中。
...
// 最后，在立即执行函数的末尾，调用了init函数来初始化页面，并定义了move、timer、prev和next函数。其中，move函数用于移动shell2到指定的item，timer函数用于启动定时器，prev函数用于切换到上一个item，next函数用于切换到下一个item。
...
(function () {
    "use strict";
    var shell2 = document.getElementsByClassName('shell2')[0];
    var slider = shell2.getElementsByClassName('shell2_slider')[0];
    var items = shell2.getElementsByClassName('item');
    var prevBtn = shell2.getElementsByClassName('prev')[0];
    var nextBtn = shell2.getElementsByClassName('next')[0];
    // 定义变量 
    var width, height, totalWidth, margin = 20,
        currIndex = 0,
        interval, intervalTime = 1500;
    function init() {
        // 初始化函数 
        resize();
    ...
    function resize() {
        // 窗口大小变化时调整大小 
        width = Math.max(window.innerWidth * .40, 675);
        ...
        for (var i = 0; i < items.length; i++) {
            let item = items[i];
            item.style.width = (width - (margin * 15)) + "px";
    ...
    var timerContainer = document.querySelector(".shell2_slider");
    timerContainer.addEventListener("mouseenter", function () {
        clearInterval(interval);
    });
    timerContainer.addEventListener("mouseleave", function () {
        timer();
    });
